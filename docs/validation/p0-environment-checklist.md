...
| P1D formal player Web | P1D/P1E | HTTPS/API and human core flow confirmed; final UI acceptance pending | Vue player page, Session/catalog/current request recovery, state and connect help, trusted static Web root. Owner completed the browser flow and identified presentation regressions for a later parity checkpoint. See [P1D](p1d.md) and [P1E](p1e.md). |
...
| P1D UI parity regression | after P1E checkpoint | local browser and automated checks passed; owner UI confirmation pending | Restored approved P-1 player layout and normal-flow presentation without changing API or business logic. True 1440/390/320 px browser checks covered idle, waiting, allocating, creating, Ready, no JoinInfo, stopping, ended, and maintenance messages. See [UI parity](p1d-parity.md). |
