...
| POST | `/server-requests/{id}/next-game` | Solo owner or current Party leader; atomically records a durable next-game intent and starts the normal stop/reclaim path; duplicate calls return the same old request |
| GET | `/server-requests/{id}/next-game` | Authorized owner reads `pending`, `paused`, or `consumed` intent, including the new request ID after consumption; `null` when no intent exists |
...
P4C next-game records intent in the same transaction that creates the stop job. A successful full reclaim creates a fresh FIFO ServerRequest exactly once with a new `requestedAt`, inheriting ArcadeGame, GamePreset, and auto/manual selection (including the manual Node). The next Allocation samples the game's then-current ContentVersion. A quarantined stop pauses the intent without creating a request; the leader's confirmed `/abandon` consumes it once while preserving old occupied capacity. If the old resource is later proved fully reclaimed before the owner decides, the request remains in the quarantine decision state until the leader confirms continuation. An ordinary `/stop` never creates a next-game intent. This flow does not call d2core instance restart.

P4D adds a lightweight `siteAnnouncement` to `/catalog`. It is player-visible independently of global maintenance and has no effect on scheduling. The administrator API and cookie remain separate from this player API.
