...
<template>
  ...
  <section v-if="mode" class="panel admin-card admin-task-workspace">
    ...
    <details class="admin-content-task admin-task-details"><summary>查看节点本地详细操作</summary><p>先在节点隔离复制 VPK，并用 Content Tool prepare、switch、status；配置 Controller 的内容元数据和正式模板绑定键。平台只显示逻辑键，不保存 DOTA_ROOT、CONTENT_ROOT、模板或 d2core 私有 data-dir 的绝对路径。</p><p>节点维护测试要使用固定 d2core v0.1.1 的正式模板。每个临时实例都须真人进入、显式 stop、查 operation/status/list 并确认完整回收，然后请求 Controller 核对。后台不会远程执行这些命令。</p><a href="https://github.com/L4C99/dota2-arcade-platform/blob/main/docs/operations.md" target="_blank" rel="noreferrer">打开完整运维手册</a></details>
  </section>
