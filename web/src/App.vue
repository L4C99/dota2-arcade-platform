    ...
    <div v-else class="active-layout">
      ...
      <aside v-if="state.phase === 'ready' && allocation?.joinInfo" class="panel join-panel">
        ...
        <div v-if="allocation.joinInfo.steamUri || allocation.joinInfo.steamChinaUri" class="protocol-entry">
          <div v-if="allocation.joinInfo.steamUri" class="protocol-entry-row"><a class="protocol-entry-button" :href="allocation.joinInfo.steamUri">Steam 一键进入</a></div>
          <div v-if="allocation.joinInfo.steamChinaUri" class="protocol-entry-row"><a class="protocol-entry-button" :href="allocation.joinInfo.steamChinaUri">蒸汽平台一键进入</a></div>
        </div>
        <div class="connect-box connect-box-primary"><div><span>手动连接 · Dota 2 控制台</span><code>{{ allocation.joinInfo.connectCommand }}</code></div><button type="button" class="copy-button" @click="copyConnect">{{ copied ? '已复制' : '复制命令' }}</button></div>
        <details v-if="allocation.joinInfo.steamUri || allocation.joinInfo.steamChinaUri" class="help protocol-troubleshooting"><summary>一键进入遇到问题？<span aria-hidden="true">⌄</span></summary><p>先启动对应客户端再试。浏览器没有反应时，可复制一键链接，在 Windows 按 <kbd>Win+R</kbd> 粘贴执行；仍无法进入时，使用上方手动连接命令。</p><div class="protocol-fallback-links"><div v-if="allocation.joinInfo.steamUri" class="protocol-fallback-link"><code>{{ allocation.joinInfo.steamUri }}</code><button type="button" class="protocol-copy-button" @click="copyProtocol(allocation.joinInfo.steamUri, 'steam')">{{ copiedProtocol === 'steam' ? '已复制 Steam 链接' : '复制 Steam 链接' }}</button></div><div v-if="allocation.joinInfo.steamChinaUri" class="protocol-fallback-link"><code>{{ allocation.joinInfo.steamChinaUri }}</code><button type="button" class="protocol-copy-button" @click="copyProtocol(allocation.joinInfo.steamChinaUri, 'steamchina')">{{ copiedProtocol === 'steamchina' ? '已复制蒸汽平台链接' : '复制蒸汽平台链接' }}</button></div></div></details>
