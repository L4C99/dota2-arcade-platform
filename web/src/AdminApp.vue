...
async function publishVersion(): Promise<void> {
  await act({ action: 'content.publish', targetId: catalogDraft.gameId, contentVersionId: catalogDraft.publishVersionId, confirmed: true }, '确认目标版本已在至少一台已恢复、兼容的节点完成准备、内容版本验收、完整回收和 Controller 核对？发布只更新新 Allocation 的目标版本，不切换节点文件，也不改变已有 Allocation；随后可开放该节点的地图分配。')
}
  ...
  <main class="admin-shell">
    ...
    <template v-else-if="overview">
      ...
      <template v-else-if="tab === 'content'">
        ...
        <section v-if="catalogDraft.gameId" class="panel admin-card"><span class="eyebrow">手动发布</span><h2>发布新开服务器使用的 VPK 版本</h2><p>当前地图：{{ gameName(catalogDraft.gameId) }} · 当前发布版本：<code>{{ overview.games.find(game => game.id === catalogDraft.gameId)?.currentContentVersionId || '尚未设置' }}</code></p><p>发布前须完成节点内容版本验收。发布只改变平台以后新分配的版本，不切换节点磁盘，也不影响已有服务器。</p><div class="admin-form-row"><label>要发布的版本<select v-model="catalogDraft.publishVersionId"><option value="" disabled>请选择</option><option v-for="version in versionsFor(catalogDraft.gameId)" :key="version.id" :value="version.id">{{ version.id }}</option></select></label><button type="button" class="primary-button" :disabled="busy || !catalogDraft.publishVersionId || catalogDraft.publishVersionId === overview.games.find(game => game.id === catalogDraft.gameId)?.currentContentVersionId" @click="publishVersion">发布为新开服版本</button></div><p v-if="catalogDraft.publishVersionId && catalogDraft.publishVersionId === overview.games.find(game => game.id === catalogDraft.gameId)?.currentContentVersionId">所选版本已经发布，无需重复发布。</p><div v-for="version in versionsFor(catalogDraft.gameId)" :key="version.id" class="admin-compact-row"><div><strong>{{ version.id }}</strong><small>SHA256 {{ version.contentSha256 }} · {{ stamp(version.createdAt) }}</small></div><span class="admin-badge">{{ overview.games.find(game => game.id === catalogDraft.gameId)?.currentContentVersionId === version.id ? '当前发布版本' : '保留版本' }}</span></div></section>
          ...
          <div class="admin-detail-grid">
            ...
            <section class="panel admin-card">
              ...
              <div v-for="binding in bindingsFor(selectedNode.id)" :key="binding.arcadeGameId" class="admin-entity admin-map-binding">
                ...
                <p class="admin-map-target">平台当前发布版本：<code>{{ targetVersionFor(binding.arcadeGameId) || '尚未设置' }}</code><span v-if="binding.reportedContentVersionId && targetVersionFor(binding.arcadeGameId) && binding.reportedContentVersionId !== targetVersionFor(binding.arcadeGameId)"> · 与节点版本不同，暂不会将此图的新服务器分配到这台节点</span></p>
                ...
                <p v-if="validationFor(selectedNode.id,binding.arcadeGameId,binding.reportedContentVersionId)" class="admin-map-validation">此节点此版本的内容版本验收已记录 · {{ stamp(validationFor(selectedNode.id,binding.arcadeGameId,binding.reportedContentVersionId)?.verifiedAt) }}</p>
                <p v-else class="admin-map-validation admin-map-validation-pending">此节点此版本尚无内容版本验收记录；完成节点真人测试与完整回收后再确认。</p>
                ...
                <div v-if="!validationFor(selectedNode.id,binding.arcadeGameId,binding.reportedContentVersionId)" class="admin-map-record">
                  ...
                  <button type="button" class="secondary-button" :disabled="busy || !validationCanBeRecorded(selectedNode,binding)" @click="act({action:'content.validate',targetId:selectedNode.id,gameId:binding.arcadeGameId,contentVersionId:binding.reportedContentVersionId,confirmed:true},'确认该节点已 Drain，目标版本完成真实 Dota 内容测试，临时实例已 stop 并完整 reclaimed/stopped/complete，Controller 已 resync 且无遗留实例？此操作仅记录内容版本验收，不修改节点内容。')">确认内容版本验收</button>
                </div>
              </div>
              <p v-if="bindingsFor(selectedNode.id).length === 0" class="admin-empty">这台节点没有地图记录；需先完成节点内容绑定和 Controller 上报。</p>
              <details class="admin-content-task admin-map-workflow"><summary>查看内容更新的详细操作</summary><p>从内容页选择“更新现有地图 VPK”，查看这张图在各节点的当前状态和下一步。节点本地命令、安全排空及回滚细节见项目运维手册；本页不会执行节点命令。</p></details>
            </section>
          </div>
